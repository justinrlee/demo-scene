...
[source,sql]
----
SELECT ACCOUNT_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE FROM ACCOUNTS_STREAM WHERE ACCOUNT_ID='a42' EMIT CHANGES;
----
...
SELECT ACCOUNT_ID, CUSTOMER_NAME, CUSTOMER_PHONE, 
        ...
        TX1_ATM, TX2_ATM, 
        DISTANCE_BETWEEN_TXN_KM, MINUTES_DIFFERENCE 
FROM ATM_POSSIBLE_FRAUD_ENRICHED EMIT CHANGES;
----
